...
export default function MarvelVillainsScreen() {
    return (
        <View style={[styles.container, { paddingTop: 0, flex: 1, backgroundColor: BAT.bg }]}>
            <HeaderBar title="Marvel Villains" />
            <View style={{ flex: 1, padding: 16 }}>
                <View style={local.headingRow}>
                    <View style={local.signalCircle}><Ionicons name="moon" size={16} color={BAT.bg} /></View>
                    <Text style={[local.headingText, { fontFamily: 'MainFont' }]}>Marvel Villains</Text>
                </View>

                <View style={local.panel}>
                    <HeroGroups publisher="Marvel Comics" alignment="bad" allowAddNotes />
                </View>
            </View>
        </View>
    );
